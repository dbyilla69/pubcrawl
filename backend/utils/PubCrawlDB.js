...
PubCrawlDB.prototype.getVideos = async function(args) {
	try {
...
PubCrawlDB.prototype.getCrawlerAuditData = async function(pubName, ids) {
	try {
...
PubCrawlDB.prototype.getCrawlerInstructionsData = async function(pubName, ids) {
	try {
		const placeholders = ids.map(id => '?').join()
...
PubCrawlDB.prototype.getPageInfo = async function({
	publisher_id,
...
PubCrawlDB.prototype.getChannelsData = async function(pubId, ids) {
	try {
		const placeholders = ids.map(id => '?').join()
